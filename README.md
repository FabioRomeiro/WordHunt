...
```bash
dk bash  # starts bash inside "wordhunt" container
...
* `dkup` uses docker-compose to start 3 containers: postgres, nginx, and wordhunt.
...
* Once `dkup` is running, `dk <command>` will run `<command>` inside the `wordhunt` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
* The wordhunt container runs 3 services:
 * django on port 8000
...
```bash
mkvirtualenv wordhunt -p python3  # creates a python3 virtualenv
...
npm run serve  # Starts nuxt frontend on port 3000
```
...
```bash
API_MOCK=0 npm run serve  # Starts nuxt frontend on port 3000
```
