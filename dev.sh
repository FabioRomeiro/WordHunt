...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=wordhunt.fabioromeiro.dev
...
workon wordhunt  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development wordhunt npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop wordhunt
    docker rm wordhunt
    docker run --name wordhunt -d --env-file /home/ubuntu/wordhunt.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/wordhunt:/dkdata \
        wordhunt start_web.sh
}
...
echo_green "Welcome to wordhunt's dev env"
